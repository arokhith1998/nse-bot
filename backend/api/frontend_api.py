...
# ── Strategy → time validity mapping (M13) ─────────────────────────────

_STRATEGY_VALIDITY = {
    "BREAKOUT": "09:30-11:00 IST",
    "MOMENTUM": "09:15-11:00 IST",
    "GAP_AND_GO": "09:15-10:00 IST",
    "MEAN_REVERSION": "11:00-13:30 IST",
    "SWING": "09:30-14:00 IST",
}

def _strategy_time_validity(strategy: Optional[str]) -> str:
    return _STRATEGY_VALIDITY.get(strategy or "", "09:30-14:45 IST")
...
async def get_picks(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    # M11: Force ETF-only mode below ₹10K capital
    if settings.capital < 10_000:
        try:
            from backend.api.etf_routes import get_etf_picks
            etf_resp = await get_etf_picks(db)
            return {
                "generated_at": dt.datetime.now(dt.timezone.utc).isoformat(),
                "data_as_of": dt.datetime.now(dt.timezone.utc).isoformat(),
                "trade_for": dt.datetime.now(dt.timezone.utc).strftime("%A %d %b %Y"),
                "universe_size": etf_resp.get("universe_size", 0),
                "scored": etf_resp.get("scored", 0),
                "skipped": 0,
                "weights": DEFAULT_WEIGHTS,
                "news_count": 0,
                "top_picks": etf_resp.get("top_picks", []),
                "stretch_picks": etf_resp.get("stretch_picks", []),
                "pre_market_watchlist": [],
                "advisory": (
                    f"At \u20b9{settings.capital:,.0f} capital, equity MIS round-trip costs "
                    f"consume >4% of risk budget. Showing ETF-only picks."
                ),
                "capital_advisory": None,
                "recommended_pick_count": etf_resp.get("recommended_pick_count", 3),
                "mode": "etf_only",
                "candidates_scanned": 0,
                "veto_breakdown": {},
                "disclaimer": (
                    "PAPER TRADING ONLY. ETF-only mode active due to low capital. "
                    "SEBI 2023: ~70% of retail intraday traders lose money."
                ),
            }
        except Exception:
            logger.warning("[api] ETF-only mode failed, falling back to equity picks")
    ...
    # M3: Fetch pre-market watchlist signals separately
    watchlist_result = await db.execute(
        select(Signal)
        .where(Signal.status == "watchlist")
        .order_by(desc(Signal.score))
    )
    watchlist_signals = watchlist_result.scalars().all()
    ...
    for sig in signals:
        ...
        pick = {
            ...
            "scale_out_1": sig.target1,  # 1R level
            "scale_out_2": sig.target2,  # 1.5R level
            "invalidation": f"Breaks below {sig.stop_loss:.2f}",
            "time_validity": _strategy_time_validity(sig.strategy),
            "ev": round(sig.ev, 2) if sig.ev is not None else None,  # real EV from scanner
        }
    ...
    # Capital-tier advisory
    capital_advisory = None
    if settings.capital < 10_000:
        capital_advisory = f"At ₹{settings.capital:,.0f} capital, round-trip costs consume >4% of your risk budget. Consider ETF-only mode."
    elif settings.capital < 25_000:
        capital_advisory = f"At ₹{settings.capital:,.0f} capital, limited to 3 high-liquidity picks to manage costs."

    # M3: Build pre-market watchlist (same shape as picks, separate field)
    pre_market_watchlist = []
    for sig in watchlist_signals[:n_picks]:
        entry = (sig.entry_zone_low + sig.entry_zone_high) / 2
        risk = entry - sig.stop_loss
        if risk <= 0:
            continue
        pre_market_watchlist.append({
            "symbol": sig.symbol,
            "price": round(entry, 2),
            "score": sig.score,
            "strategy": sig.strategy,
            ...
            "stop_loss": sig.stop_loss,
            "target": sig.target1,
            "ev": round(sig.ev, 2) if sig.ev is not None else None,
        })

    # M6: Import veto breakdown from scanner
    from backend.modules.scanner import last_veto_breakdown, last_candidates_scanned

    # M15: Detect correlated pairs among displayed picks
    correlated_pairs = []
    all_display_symbols = [p["symbol"] for p in top_picks + stretch_picks]
    for pair_set in _CORRELATED_PAIRS:
        pair_syms = [s for s in pair_set if s in all_display_symbols]
        if len(pair_syms) >= 2:
            correlated_pairs.append(pair_syms[:2])
    ...
    return {
        ...
        "candidates_scanned": last_candidates_scanned,
        "veto_breakdown": last_veto_breakdown,
...
async def get_regime(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    # Strategy gating based on regime
    REGIME_STRATEGIES = {
        "TRENDING_UP": {"allowed": ["Breakout", "Momentum", "Gap & Go", "Swing"], "disallowed": ["Mean Reversion", "Shorts"]},
        "TRENDING_DOWN": {"allowed": ["Mean Reversion"], "disallowed": ["Breakout", "Momentum", "Gap & Go", "All Longs"]},
        "RANGE_BOUND": {"allowed": ["Mean Reversion", "Swing"], "disallowed": ["Breakout"]},
        "HIGH_VOL": {"allowed": ["Swing (reduced size)"], "disallowed": ["Breakout", "Momentum"]},
    ...
    return {
        ...
        "allowed_strategies": strategies["allowed"],
        "disallowed_strategies": strategies["disallowed"],
    }
...
async def get_performance(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    r_multiples = []
    for trade, signal in rows:
        risk = abs(signal.stop_loss - ((signal.entry_zone_low + signal.entry_zone_high) / 2))
        if risk > 0:
            r = (trade.net_pnl or 0) / (risk * trade.qty) if trade.qty > 0 else 0
            r_multiples.append(round(r, 2))

    # Bucket R-multiples
    r_buckets = {"<-1R": 0, "-1R to -0.5R": 0, "-0.5R to 0": 0, "0 to 0.5R": 0, "0.5R to 1R": 0, "1R to 1.5R": 0, "1.5R to 2R": 0, ">2R": 0}
    for r in r_multiples:
        if r < -1: r_buckets["<-1R"] += 1
        elif r < -0.5: r_buckets["-1R to -0.5R"] += 1
        elif r < 0: r_buckets["-0.5R to 0"] += 1
        elif r < 0.5: r_buckets["0 to 0.5R"] += 1
        elif r < 1: r_buckets["0.5R to 1R"] += 1
        elif r < 1.5: r_buckets["1R to 1.5R"] += 1
        elif r < 2: r_buckets["1.5R to 2R"] += 1
        else: r_buckets[">2R"] += 1

    expectancy = round(sum(r_multiples) / len(r_multiples), 3) if r_multiples else 0

    # M17: MAE/MFE distributions (approximate from entry/exit/stop/target)
    mae_values = []  # max adverse excursion as % of entry
    mfe_values = []  # max favorable excursion as % of entry
    for trade, signal in rows:
        entry = (signal.entry_zone_low + signal.entry_zone_high) / 2
        if entry <= 0:
            continue
        # MAE: worst case was at least to stop loss if trade was a loser
        if (trade.net_pnl or 0) < 0:
            mae = abs(entry - signal.stop_loss) / entry * 100
        else:
            # Winner: MAE was some fraction of stop distance
            mae = abs(entry - signal.stop_loss) / entry * 100 * 0.3  # estimate
        mfe = abs((trade.exit_price or entry) - entry) / entry * 100
        if (trade.net_pnl or 0) > 0:
            mfe = max(mfe, abs(signal.target1 - entry) / entry * 100)
        mae_values.append(round(mae, 2))
        mfe_values.append(round(mfe, 2))

    # Bucket MAE/MFE
    def _bucket_excursion(values, label):
        buckets = {f"0-0.5%": 0, f"0.5-1%": 0, f"1-2%": 0, f"2-3%": 0, f">3%": 0}
        for v in values:
            if v < 0.5: buckets["0-0.5%"] += 1
            elif v < 1: buckets["0.5-1%"] += 1
            elif v < 2: buckets["1-2%"] += 1
            elif v < 3: buckets["2-3%"] += 1
            else: buckets[">3%"] += 1
        return buckets

    mae_distribution = _bucket_excursion(mae_values, "MAE") if mae_values else {}
    mfe_distribution = _bucket_excursion(mfe_values, "MFE") if mfe_values else {}
...
# ── POST /api/picks/{symbol}/veto ──────────────────────────────────────

@router.post("/api/picks/{symbol}/veto")
async def veto_pick(symbol: str, reason: str = Query(default="other"), db: AsyncSession = Depends(get_db)):
    """Record a user veto for a pick."""
    veto = UserVeto(
        symbol=symbol.upper(),
        reason=reason,
    ...
    db.add(veto)
    await db.commit()
    return {"status": "ok", "symbol": symbol, "reason": reason}
