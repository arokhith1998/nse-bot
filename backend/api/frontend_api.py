...
async def get_picks(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    # Split into top picks (first 6) and stretch picks (next 6)
    all_picks = []
    for sig in signals:
        ...
        qty = max(1, int((settings.capital * settings.risk_per_trade_pct / 100) / risk)) if risk > 0 else 1
